...
const CajaComentario = (props: Props) => {
    ...
    const setDimention = () => {
        if (refComentario.current) {
            ...
            if (refDifumino.current)
                refDifumino.current.classList.add('d-none');
            if (refBtn.current)
                refBtn.current.classList.add('d-none');
        }
    }
    ...
    return (
        <>
            <div className="comentario position-relative">
                <p className="pe-4" style={{ textAlign: "justify" }} ref={node => refComentario.current = node}>
                </p>
                <div ref={node => refDifumino.current = node} className="position-absolute difumino">
                </div>
            </div>
            <button onClick={() => verMas()} className="btn btn-sm ms-4" ref={node => refBtn.current = node} style={{ color: "var(--azul-oscuro)" }}><FaChevronDown /> Ver más</button>
        </>
    )
}
